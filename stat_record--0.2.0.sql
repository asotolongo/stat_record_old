...
CREATE OR REPLACE FUNCTION _stat_record.detail_record(id bigint, lim integer DEFAULT 10)
 RETURNS SETOF text
 LANGUAGE plpgsql
AS $function$
declare 
...
begin 
  ...
  for result in ( select '  '||'Configuration: '
   ...
   for result in ( select '  '|| lim::text ||' Querys with more call: '
               union all
   ...
   for result in ( select '  '|| lim::text ||' Querys with more total time: '
               union all
    ...
    for result in ( select '  '|| lim::text ||' Querys with more mean time: '
               union all
    ...
    for result in ( select '  '|| lim::text ||' Querys with  max time: '
               union all
    ...
    for result in ( select '  '|| lim::text ||' Querys with  more row  returned: '
               union all
   ...
   for result in ( select '  '|| lim::text ||' table bloat: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='bloat' and var_name='table_bloat' limit 10 ) loop
     RETURN NEXT result ;
    end loop;
    ...
    for result in ( select '  '|| lim::text ||' index bloat: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='bloat' and var_name='index_bloat' limit 10 ) loop
     RETURN NEXT result ;
    end loop;
...
CREATE OR REPLACE FUNCTION _stat_record.total_report_for_2last_record(lim integer DEFAULT 10)
...
CREATE OR REPLACE FUNCTION _stat_record.total_report_for_amonth_record(p_month date DEFAULT date(now()), lim integer DEFAULT 10)
 RETURNS SETOF _stat_record.global_report_record
 LANGUAGE plpgsql
AS $function$
declare 
