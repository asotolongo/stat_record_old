  ...
  for result in ( select '  '||'Configuration: '
   ...
   for result in ( select '  '|| lim::text ||' Queries with more call: '
               ...
               select '    '||'db: '|| bd||' -- query: ' ||query_text||' -- calls: '||stat_val from ( select * from _stat_record._query_stat where id_Record =$1 order by stat_val::bigint desc )  as sub 
                where id_Record =$1 and stat_name='calls'  limit $2+1 ) loop
    RETURN NEXT result ;
   end loop;
   ...
   for result in ( select '  '|| lim::text ||' Queries with more total time: '
               ...
               select '    '||'db: '|| bd||' -- query: ' ||query_text||' -- total_time: '||stat_val ||' ms' from ( select * from _stat_record._query_stat  where id_Record =$1 order by stat_val::numeric desc )  as sub 
                where id_Record =$1 and stat_name='total_time'  limit $2+1 ) loop
    RETURN NEXT result ;
   end loop; 
    ...
    for result in ( select '  '|| lim::text ||' Queries with more mean time: '
               ...
               select '    '||'db: '|| bd||' -- query: ' ||query_text||' -- mean_time: '||stat_val ||' ms' from ( select * from _stat_record._query_stat  where id_Record =$1 order by stat_val::numeric desc )  as sub 
                where id_Record =$1 and stat_name='mean_time'  limit $2+1 ) loop
     RETURN NEXT result ;
    end loop;
    ...
    for result in ( select '  '|| lim::text ||' Queriess with  max time: '
               ...
               select '    '||'db: '|| bd||' -- query: ' ||query_text||' -- max_time: '||stat_val ||' ms' from ( select * from _stat_record._query_stat where id_Record =$1 order by stat_val::numeric desc )  as sub 
                where id_Record =$1 and stat_name='max_time'  limit $2+1 ) loop
     RETURN NEXT result ;
    end loop;
    ...
    for result in ( select '  '|| lim::text ||' Queries with least cache ratio: '
               ...
               select '    '||'db: '|| bd||' -- query: ' ||query_text||' -- hit_percent: '||stat_val  from ( select * from _stat_record._query_stat where id_Record =$1 order by stat_val::numeric  nulls last )  as sub 
                where id_Record =$1 and stat_name='hit_percent'  limit $2+1 ) loop
     RETURN NEXT result ;
    end loop;
    ...
    for result in ( select '  '|| lim::text ||' tables Weigth: '
               ...
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='table' and var_name='weigth' limit $2+1 ) loop
     RETURN NEXT result ;
    end loop;
    ...
    for result in ( select '  '|| lim::text ||' tables with more estimated tuples: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='table' and var_name='reltuples' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
    ...
    for result in ( select '  '|| lim::text ||' tables with more Inserted tuples: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='table' and var_name='n_tup_ins' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
     ...
     for result in ( select '  '|| lim::text ||' tables with more Updated tuples: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='table' and var_name='n_tup_upd' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
     ...
     for result in ( select '  '|| lim::text ||' tables with more Deleted tuples: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='table' and var_name='n_tup_del' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
     ...
     for result in ( select '  '|| lim::text ||' tables with more Autovacuum: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='maintenance' and var_name='autovacuum_count' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
      ...
      for result in ( select '  '|| lim::text ||' tables with more Manual Vacuum: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='maintenance' and var_name='vacuum_count' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
     ...
     for result in ( select '  '|| lim::text ||' tables with more Auto Analyze: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='maintenance' and var_name='autoanalyze_count' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
      ...
      for result in ( select '  '|| lim::text ||' tables with more Manual Analyze: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='maintenance' and var_name='analyze_count' limit $2+1 ) loop
     RETURN NEXT result ;
     end loop;
     ...
     for result in ( select '  '|| lim::text ||' indexs Weigth: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='index' and var_name='index_size' limit $2+1 ) loop
     RETURN NEXT result ;
    end loop;
    ...
    for result in ( select '  '|| lim::text ||' indexs used: '
               union all
               select '    '|| objname ||': '||var_val ||' '||um  from ( select * from _stat_record._db_stat where id_Record =$1 order by var_val::numeric desc )  as sub 
                where id_Record =$1 and objtyp='index' and var_name='times_used' limit $2+1 ) loop
     RETURN NEXT result ;
    end loop;
...
begin 
 ...
 --database version
  ...
  INSERT INTO _stat_record._global_stat select id,1,'Databases tempfiles',tempfiles,'' from sub
  ;
 
 --bgwriter stats
  with sub as (select * from pg_stat_bgwriter ),
  
  cpt as (INSERT INTO _stat_record._global_stat select id,1,'Checkpoints_timed',checkpoints_timed,'' from sub),
  cpr as (INSERT INTO _stat_record._global_stat select id,1,'Checkpoints_req',checkpoints_req,'' from sub),
  cpwt as (INSERT INTO _stat_record._global_stat select id,1,'Checkpoint_write_time',checkpoint_write_time,'' from sub),
  cpst as (INSERT INTO _stat_record._global_stat select id,1,'Checkpoint_sync_time',checkpoint_sync_time,'' from sub),
  bcp as (INSERT INTO _stat_record._global_stat select id,1,'Buffers_checkpoint',buffers_checkpoint,'' from sub),
  bc as (INSERT INTO _stat_record._global_stat select id,1,'Buffers_clean',buffers_clean,'' from sub),
  mc as (INSERT INTO _stat_record._global_stat select id,1,'Maxwritten_clean',maxwritten_clean,'' from sub),
  bb as (INSERT INTO _stat_record._global_stat select id,1,'Buffers_backend',buffers_backend,'' from sub),
  bbs as (INSERT INTO _stat_record._global_stat select id,1,'Buffers_backend_fsync',buffers_backend_fsync,'' from sub)
  INSERT INTO _stat_record._global_stat select id,1,'Buffers_alloc',buffers_alloc,'' from sub
  ;
 ...
 --checking pg_stat_statemets
  select * into reg from pg_available_extensions where name ='pg_stat_statements' and installed_version is not null and  position ('pg_stat_statements' in current_setting('shared_preload_libraries') ) >0;
   if found then 
	  ...
	  hit as (INSERT INTO _stat_record._query_stat select id,datname,queryid,query,'hit_percent',round(hit_percent,2) from sub)
